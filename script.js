...
/* =========================================
  LÓGICA REPRODUCTOR EXPANDIBLE (MÓVIL)
  Designed by DIGITALITY
  =========================================
*/
...
// Elementos de la vista expandida para actualizar
...
const durationTimeExpanded = document.getElementById('duration-time-expanded');

// 1. Funciones de Expansión/Colapso
function expandPlayer() {
    // Solo expandir si estamos en móvil (ancho < 768px)
    if (window.innerWidth <= 768) {
        playerFooter.classList.add('expanded');
    }
}

function collapsePlayer() {
    playerFooter.classList.remove('expanded');
}

// 2. Modificar la función loadSong existente para actualizar AMBAS vistas
// Busca tu función loadSong(index) y reemplázala o modifícala:

function loadSong(index) {
    // ... (Mantén tu lógica existente de resaltado verde en la tabla) ...

    songIndex = index;
    const song = songs[songIndex];

    // Actualizar VISTA MINI (Existente)
    document.getElementById('footer-title').innerText = song.title;
    document.getElementById('footer-artist').innerText = song.artist;
    document.getElementById('footer-img').src = song.image;

    // Actualizar VISTA EXPANDIDA (NUEVO)
    expandedTitle.innerText = song.title;
    expandedArtist.innerText = song.artist;
    expandedImg.src = song.image;
...
// 3. Modificar playSong/pauseSong para sincronizar iconos
// Busca tus funciones playSong/pauseSong y modifícalas:

function playSong() {
    audio.play();
    // Icono grande (existente)
    playPauseBtn.classList.replace('fa-play-circle', 'fa-pause-circle');
    // Icono mini (NUEVO)
    miniPlayBtn.classList.replace('fa-play', 'fa-pause');
}
...
function pauseSong() {
    ...
    playPauseBtn.classList.replace('fa-pause-circle', 'fa-play-circle');
    // Icono mini (NUEVO)
    miniPlayBtn.classList.replace('fa-pause', 'fa-play');
}

// 4. Sincronizar Barras de Progreso
// Busca tu evento audio.addEventListener('timeupdate', ...) y modifícalo:
...
audio.addEventListener('timeupdate', () => {
    ...
    if (duration) {
        ...
        // Actualizar barra mini (existente)
        progressFill.style.width = `${percent}%`;
        document.getElementById('current-time').innerText = formatTime(currentTime);
        document.getElementById('duration-time').innerText = formatTime(duration);

        // Actualizar barra expandida (NUEVO)
        progressFillExpanded.style.width = `${percent}%`;
        currentTimeExpanded.innerText = formatTime(currentTime);
        durationTimeExpanded.innerText = formatTime(duration);
    }
});

// 5. Hacer interactiva la barra de progreso expandida
const progressContainerExpanded = document.getElementById('progress-container-expanded');
progressContainerExpanded.addEventListener('click', (e) => {
    const width = progressContainerExpanded.clientWidth;
    const clickX = e.offsetX;
    const duration = audio.duration;
    audio.currentTime = (clickX / width) * duration;
});